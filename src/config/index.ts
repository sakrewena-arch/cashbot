...
// Utilise PORT (Railway) ou API_PORT
...
export const BOT_MESSAGES = {
  ...
  balanceInfo: function(balance: string, pending: string, totalEarned: string): string {
    return ['💰 *Ton Solde*', '', '💰 Disponible : *' + balance + ' €*', '⏳ En attente : *' + pending + ' €*', '📈 Total gagné : *' + totalEarned + ' €*'].join('\n');
  },
  taskList: function(tasks: any[]): string {
    if (tasks.length === 0) return "📭 *Aucune tâche disponible pour le moment.*\n\nReviens plus tard !";
    var taskLines = tasks.map(function(t: any, i: number): string { return (i + 1) + '. ' + t.icon + ' *' + t.title + '* - ' + t.reward + ' €'; });
    return '🎯 *Tâches Disponibles*\n\n' + taskLines.join('\n');
  },
  taskDetail: function(task: any): string {
    var lines = ['*' + task.icon + ' ' + task.title + '*', '', '📝 *Description :*', task.description || 'Aucune description', '', '💰 *Récompense :* ' + task.reward + ' €', '👥 *Participants :* ' + task.currentParticipants + '/' + (task.maxParticipants || '∞'), '📅 *Fin :* ' + (task.endDate ? new Date(task.endDate).toLocaleDateString('fr-FR') : 'Illimitée')];
  ...
  channelsRequired: ['⚠️ *Accès restreint*', '', "Tu dois d'abord rejoindre tous nos canaux obligatoires pour accéder au menu principal.", '', 'Utilise /start pour vérifier.'].join('\n'),
  profileInfo: function(user: any): string {
    return ['👤 *Ton Profil*', '', '🆔 ID : `' + user.telegramId + '`', '👤 Nom : ' + (user.firstName || '') + ' ' + (user.lastName || ''), '📛 Pseudo : @' + (user.username || 'Non défini'), '🌍 Langue : ' + user.languageCode, '📅 Inscrit le : ' + new Date(user.createdAt).toLocaleDateString('fr-FR'), '🔗 Code parrain : `' + user.referralCode + '`', '👥 Filleuls : ' + user.referralCount, '💰 Gagné avec parrainage : ' + user.referralEarnings + ' €'].join('\n');
  },
  referralInfo: function(user: any, referralLink: string): string {
    return ['👥 *Parrainage*', '', '🔗 *Ton lien de parrainage :*', '`' + referralLink + '`', '', '📊 *Tes statistiques :*', '👥 Filleuls : ' + user.referralCount, '💰 Gains parrainage : ' + user.referralEarnings + ' €', '', '💡 *Partage ton lien et gagne 10% des gains de tes filleuls !*', '', "Invite tes amis à utiliser le bot et gagne de l'argent !"].join('\n');
  },
  ...
  taskCompleted: function(task: any, reward: string): string { return ['✅ *Tâche complétée !*', '', '🎯 ' + task.title, '💰 Récompense : *' + reward + ' €*', '', "La récompense a été créditée sur ton solde."].join('\n'); },
  taskPending: function(task: any): string { return ['⏳ *Tâche soumise !*', '', '🎯 ' + task.title, '', "Ta preuve a été envoyée à l'équipe de validation.", 'Tu seras notifié dès que ta tâche sera validée.'].join('\n'); },
  taskRejected: function(task: any, reason: string): string { return ['❌ *Tâche refusée*', '', '🎯 ' + task.title, '📝 Raison : ' + (reason || 'Aucune raison fournie'), '', 'Tu peux réessayer !'].join('\n'); },
  withdrawalRequested: function(amount: string): string { return ['✅ *Demande de retrait soumise*', '', '💰 Montant : ' + amount + ' €', '⏳ Statut : En attente de validation', '', 'Tu seras notifié dès que ton retrait sera traité.'].join('\n'); },
  withdrawalApproved: function(amount: string): string { return ['✅ *Retrait approuvé !*', '', '💰 ' + amount + ' € a été envoyé sur ton compte.', '', 'Merci de ta confiance !'].join('\n'); },
  withdrawalRejected: function(amount: string, reason: string): string { return ['❌ *Retrait refusé*', '', '💰 ' + amount + ' €', '📝 Raison : ' + (reason || 'Aucune raison fournie'), '', 'Contacte le support pour plus d\'informations.'].join('\n'); },
  dailyBonusClaimed: function(amount: string, streak: number): string { return ['🎁 *Bonus quotidien réclamé !*', '', '💰 +' + amount + ' €', '🔥 Série : ' + streak + ' jours', '', 'Reviens demain pour ton prochain bonus !'].join('\n'); },
  promoCodeUsed: function(code: string, reward: string): string { return ['🎟 *Code promo utilisé !*', '', 'Code : ' + code, '💰 Récompense : +' + reward + ' €'].join('\n'); },
  newReferral: function(username: string, reward: string): string { return ['👥 *Nouveau filleul !*', '', username + ' a rejoint Cashbot grâce à toi !', '💰 Bonus parrainage : +' + reward + ' €'].join('\n'); },
